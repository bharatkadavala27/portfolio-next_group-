                            ...
                            <div class="exzoom" id="exzoom">
                                <div class="exzoom_img_box">
                                    <ul class='exzoom_img_ul'>
                                        ...
                                        if (stripos($image, 'uploads/products/') !== 0 && stripos($image,
                                        'Uploads/products/') !== 0) {
        ...
        <i class="fa fa-chevron-right"></i>
                ...
                <div class="col" id="product-details">
                    <div class="bg-white p-4 rounded mb-3">
                        ...
                        @php $descriptionContentCheck = strip_tags($product->description ?? ''); @endphp
                        @if (!empty($descriptionContentCheck))
                            ...
                            <div class="card h-100 document-card">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex flex-column gap-2 d-flex-custom btn ">
                                        @if($document->hasFile())
                                        <a href="{{ asset($document->file_path) }}" class="" target="_blank" download>
                                            <i
                                                class="fa {{ Str::endsWith($document->file_path, '.pdf') ? 'fa-file-pdf-o' : 'fa-file-o' }} me-1"></i>
                                            {{ $document->document_name ?? basename($document->file_path) }}
                                            <i class="fa fa-download ms-1"></i>
                                        </a>
                                        @endif
                                        @if($document->hasLink())
                                        <a href="{{ $document->path }}" class="" target="_blank">
                                            <i
                                                class="fa {{ Str::endsWith($document->path, '.pdf') ? 'fa-file-pdf-o' : 'fa-file-o' }} me-1"></i>
                                            {{ $document->document_name ?? basename(parse_url($document->path,
                                            PHP_URL_PATH)) }}
                                            <i class="fa fa-download ms-1"></i>
                                        </a>
                                        @endif
                    ...
                    <h5 class="mb-1 fs-6 fw-bold">
                        ...
                        @elseif($document->path)
                            <a href="{{ $document->path }}"
                               target="_blank"
                               class="text-decoration-none text-primary download border-0">
                                {{ $document->document_name ?? basename(parse_url($document->path, PHP_URL_PATH)) }}
                            </a>
                        @else
                            <span class="text-muted">{{ $document->document_name ?? 'No document available' }}</span>
                        @endif
                    </h5>
                    <p class="text-muted mb-0 small">
                        @php
                            ...
                            if ($document->file_path) {
                                ...
                                $fileType = $document->type ?? strtoupper(pathinfo(parse_url($document->path, PHP_URL_PATH), PATHINFO_EXTENSION)) ?? 'LINK';
                            }
                        @endphp
                        <i class="fa {{ $fileType == 'PDF' ? 'fa-file-pdf-o' : ($document->path ? 'fa-external-link' : 'fa-file-o') }} me-1"></i>
...
<script>
    ...
    if (exzoomElement && typeof $ !== 'undefined' && $.fn.exzoom) {
        try {
            const productDetailsWidth = $('#product-details').length ? $('#product-details').outerWidth() : 400;
            $("#exzoom").exzoom({
                navWidth: 60,
                navHeight: 60,
                navItemNum: 5,
    ...
    document.querySelectorAll('.description-content').forEach(content => {
        ...
        content.style.maxHeight = originalInlineStyles.maxHeight;
        content.style.webkitLineClamp = originalInlineStyles.webkitLineClamp;
        content.style.display = originalInlineStyles.display;
        content.style.overflow = originalInlineStyles.overflow;
        content.style.webkitBoxOrient = originalInlineStyles.webkitBoxOrient;
        ...
        if (actualScrollHeight > collapsedMaxHeight + (lineHeight * 0.5) ) {
            ...
            content.style.maxHeight = 'none';
            content.style.webkitLineClamp = 'unset';
            content.style.display = originalInlineStyles.display || 'block';
            content.style.overflow = originalInlineStyles.overflow || 'visible';
            content.style.webkitBoxOrient = 'auto';
        }
    });

    // Handle Specifications Read More/Read Less (SINGLE GLOBAL BUTTON)
    ...
    if (toggleAllSpecsBtn) {
        ...
        toggleAllSpecsBtn.addEventListener('click', function() {
            ...
            document.querySelectorAll('.attribute-hidden').forEach(attribute => {
                attribute.style.display = allSpecsAreExpanded ? 'block' : 'none';
            });

            // Show/hide additional specifications in all attributes (Desktop tables)
            document.querySelectorAll('.spec-table-body-hidden').forEach(hiddenBody => {
                ...
                hiddenBody.querySelectorAll('.spec-row').forEach(row => {
                    if (allSpecsAreExpanded) {
                        row.classList.remove('spec-row-hidden');
                    } else {
                        row.classList.add('spec-row-hidden');
                    }
                });
            });

            // Show/hide additional specifications in mobile lists
